    ...
    const fetchDoctorData = async () => {
      try {
        ...
        const doctorRes = await fetch(
          `https://mediconnet-backend.onrender.com/api/doctors/getStaffAccount/${userData.userId}`,
          { credentials: "include" }
        );
        ...
        if (doctorData.assignedPatientID?.length > 0) {
          const patientPromises = doctorData.assignedPatientID.map(patientId => 
            fetch(`https://mediconnet-backend.onrender.com/api/patients/${patientId.$oid}`, {
              credentials: "include"
            }).then(res => {
